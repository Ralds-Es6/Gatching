    ...
    return (
        <div className="min-h-screen bg-[#6D8FA3] text-white">
            <div className="container py-12">
                <div className="max-w-6xl mx-auto">
                    ...
                    <header className="flex items-center gap-8 mb-12">
                        <h1 className="text-4xl font-normal tracking-tight flex items-center gap-3 font-['Balerina']">
                            <span className={`w-2.5 h-10 rounded-full transition-colors duration-500 ${activeView === 'user' ? 'bg-primary' : 'bg-accent'}`}></span>
                        ...
                        <nav className="flex gap-6 items-center ml-auto">
                            <button
                                ...
                                title="Hannyangi"
                                className={`relative w-16 h-16 rounded-full flex items-center justify-center transition-all duration-500 group ${activeView === 'user' ? 'scale-110' : ''}`}
                            >
                                ...
                                <div className="w-14 h-14 rounded-full overflow-hidden bg-white/5 border border-white/10 relative z-10">
                            ...
                            <button
                                ...
                                title="Jaeguri"
                                className={`relative w-16 h-16 rounded-full flex items-center justify-center transition-all duration-500 group ${activeView === 'super' ? 'scale-110' : ''}`}
                            >
                    ...
                    <main className="space-y-12">
                            ...
                            <div className="w-full relative overflow-hidden rounded-3xl border border-white/10 group shadow-2xl animate-fadeIn">
                                ...
                                <div className="absolute inset-0 bg-gradient-to-t from-black/95 via-black/30 to-transparent flex items-end p-12">
                                    <div className="space-y-4">
                                        <h2 className="text-7xl font-normal text-[#A7D2EB] drop-shadow-2xl font-['Balerina']">Alchemist Hannyangi</h2>
                                        <p className="text-white/80 text-lg max-w-2xl leading-relaxed">Wish now to obtain premium potions specially brewed by Alchemist Hannyangi!</p>
                                        <button
                                            onClick={handleWishHannyangi}
                                            disabled={isWishing || hasWishedHannyangi}
                                            className="bg-[#B5CDFF] hover:bg-[#a0bbff] text-[#DAE8FF] px-10 py-4 rounded-xl text-sm font-black uppercase tracking-widest mt-4 transition-all shadow-[0_4px_15px_rgba(181,205,255,0.3)] disabled:opacity-50 disabled:cursor-not-allowed"
                                        >
                                            {isWishing && activeView === 'user' ? 'Wishing...' : hasWishedHannyangi ? 'WISH COMPLETED' : 'WISH'}
                                        </button>
                                        {hasWishedHannyangi && (
                                            <p className="text-xs text-[#A7D2EB]/60 font-medium italic mt-2 animate-fadeIn"> You have claimed your one-time potion from Hannyangi. </p>
                                        )}
                                    </div>
                                </div>
                            </div>
                        ...
                        {activeView === 'super' && (
                            <div className="w-full relative overflow-hidden rounded-3xl border border-white/10 group shadow-2xl animate-fadeIn">
                                ...
                                <div className="absolute inset-0 bg-gradient-to-t from-black/95 via-black/30 to-transparent flex items-end p-12">
                                    <div className="space-y-4">
                                        <h2 className="text-7xl font-normal text-[#EBB1BB] drop-shadow-2xl font-['Balerina']">Appraiser Jaeguri</h2>
                                        <p className="text-white/80 text-lg max-w-2xl leading-relaxed">Wish now to obtain premium potions specially valued by Appraiser Jaeguri!</p>
                                            ...
                                            {!isJaeguriUnlocked ? (
                                                <div className="space-y-4 animate-fadeIn">
                                                    <div className="relative group/input">
                                                        <input
                                                            ...
                                                            className="bg-black/40 border border-[#FEABC5]/30 rounded-xl px-6 py-4 w-64 text-sm font-medium focus:outline-none focus:border-[#FEABC5]/60 transition-all placeholder-[#FEABC5]/40"
                                                        />
                                                        <span className="absolute right-4 top-1/2 -translate-y-1/2 text-xl opacity-50 text-white">🔒</span>
                                                    </div>
                                                    <p className="text-sm text-[#EBB1BB]/60 font-medium italic tracking-wide">Jaeguri requires a secret enchantment to proceed...</p>
                                                </div>
                                            ) : (
                                                <div className="flex flex-col gap-2 animate-scaleUp">
                                                    ...
                                                    <button
                                                        ...
                                                        className="bg-[#FEABC5] hover:bg-[#fd8dae] text-[#FFD4E1] px-12 py-4 rounded-xl text-sm font-black uppercase tracking-widest transition-all shadow-[0_4px_25px_rgba(254,171,197,0.4)] disabled:opacity-50 disabled:cursor-not-allowed"
                                                    >
                                                        {isWishing && activeView === 'super' ? 'Wishing...' : hasWishedJaeguri ? 'WISH COMPLETED' : 'WISH'}
                                                    </button>
                                                    {hasWishedJaeguri && (
                                                        <p className="text-xs text-[#EBB1BB]/60 font-medium italic mt-2 animate-fadeIn"> You have claimed your one-time gift from Jaeguri. </p>
                                                    )}
                                                </div>
                                            )}
                                        </div>
                        ...
                        <div className="flex justify-center">
                            <button
                                ...
                                className={`px-8 py-3 rounded-full text-sm font-bold transition-all border ${activeView === 'user'
                        ...
                        {/* Inventory Section */}
                        {showInventory && (
                            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-6 animate-fadeInUp">
                                {inventory.filter(item => item.pool === (activeView === 'user' ? 'hannyangi' : 'jaeguri')).length > 0 ? (
                                    inventory
                                        ...
                                        .map((item, index) => (
                                            <div key={index} className="group relative bg-white/5 rounded-2xl p-4 border border-white/10 hover:border-white/30 transition-all">
                                                <div
                                                    className="relative cursor-pointer overflow-hidden rounded-lg mb-4 group/card"
                                                    onClick={() => {
                                                ...
                                                <div className="flex gap-2">
                                                    <button
                                                        ...
                                                        className="flex-1 bg-white/10 hover:bg-white/20 py-2 rounded-lg text-xs font-bold transition-all text-white/70 hover:text-white"
                                                    >
                                                        Save
                                                    </button>
                                                </div>
                ...
                <div className="fixed inset-0 z-[200] flex items-center justify-center bg-black/60 backdrop-blur-sm animate-fadeIn">
                    <div className="text-center space-y-6">
                        <div className="relative">
                            ...
                            <span className="text-4xl absolute -top-4 -right-4 animate-bounce">✨</span>
                            <span className="text-4xl absolute -bottom-4 -left-4 animate-bounce delay-100">✨</span>
                        </div>
                        <p className="text-2xl font-black tracking-widest uppercase animate-pulse">Wishing upon the stars...</p>
                    </div>
                </div>
            )}
                ...
                <div className="fixed inset-0 z-[300] flex items-center justify-center p-6 bg-black/80 backdrop-blur-md animate-fadeIn">
                    <div className="relative max-w-lg w-full bg-[#1a1a1a] rounded-[32px] overflow-hidden shadow-[0_0_50px_rgba(255,255,255,0.1)] border border-white/10 animate-scaleUp">
                        <div className="p-8 space-y-6 text-center">
                            <h2 className="text-3xl font-black bg-gradient-to-r from-[#A7D2EB] to-[#B5CDFF] bg-clip-text text-transparent">New Card Obtained!</h2>
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                <button
                                    ...
                                    className="bg-white/10 hover:bg-white/20 py-4 rounded-xl text-sm font-bold transition-all border border-white/5"
                                >
                                    💾 Save to Device
                                </button>
                                <button
                                    onClick={() => setWishResult(null)}
                                    className="bg-primary hover:brightness-110 py-4 rounded-xl text-sm font-black transition-all shadow-lg shadow-primary/20"
                                >
                                    Close
                                </button>
                            </div>
            ...
            {previewImage && (
                ...
                >
                    <div
                        className="relative max-w-4xl w-full h-full flex flex-col items-center justify-center gap-8 animate-scaleUp"
                        onClick={(e) => e.stopPropagation()}
                    >
                        <button
                            onClick={() => setPreviewImage(null)}
                            className="absolute -top-12 right-0 md:-right-12 text-white/60 hover:text-white text-4xl transition-colors"
                        >
                            ✕
                        </button>

                        <div className="relative group w-full max-h-[70vh] flex justify-center">
                            <img
                        ...
                        <div className="flex flex-col items-center gap-4">
                            <h3 className="text-2xl font-['Balerina'] text-white tracking-widest uppercase">{previewImage}</h3>
                            <button
                                onClick={() => downloadImage(previewImage, previewFolder)}
                                className="bg-white/10 hover:bg-white/20 px-12 py-4 rounded-full text-sm font-black tracking-widest uppercase transition-all border border-white/10 flex items-center gap-3 group"
                            >
